...
internal class NotificationService
{
    ...
    {
        var exceptionModel = new NotifyExceptionModel
        {
            Exception = exception
        };

        if (!string.IsNullOrEmpty(errorDescriptionKey))
            exceptionModel.Description = LocalizationResourcesUtils.GetValue("Notifications", errorDescriptionKey);

        NotifyWithSpecialContent(LocalizationResourcesUtils.GetValue("Notifications", errorTitleKey), "ExceptionNotifyTemplate", exceptionModel, "\uE711", 20 * 1000);
    }
    ...
    private ContentPresenter CreateNotifyPresenterWithoutContent(string title, string icon = "\uE7E7", int delay = 5000)
    {
        ...
        async void ContentPresenter_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            var notifyPresenterModel = new NotifyPresenterModel
            {
                Icon = icon,
                NotifyTitle = title,
                NotifyContent = null,
            ...
            contentPresenter.DataContext = notifyPresenterModel;

            await Task.Delay(delay);

            if (!notifyPresenterModel._removed)
                await notifyPresenterModel.Close();
        }
